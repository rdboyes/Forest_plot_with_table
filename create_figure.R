...
# using Fira Sans font - this will need to be installed 
...
tdata_print <- select(tdata, Subgroup, Inclisiran, Placebo, ` `, `Estimate (95% CI)`)

# add a row numbers for graph data
...
center <- ggplot(data = gdata, aes(y = row_num, x = Estimate)) +
  ...
  geom_errorbarh(aes(y = row_num, xmin = `CI low`, xmax = `CI high`), height = .25) +
  theme_classic() +
  scale_y_continuous(expand = c(0,0), limits = c(-.65, 30.7)) +
...
t1 <- ttheme_minimal(core=list(
  ...
  colhead = list(fg_params = list(hjust = 0, x = 0.05, fontfamily = "Fira Sans"),
...
# in the right spot
# under both, the arrows and labels
...
ggsave(dpi = 600, height = 10.2, width = 11, units = "in", filename = "final_plot.png")
